/* ==========================================================================
   js/core-app.js - MOTOR LÓGICO MATEMÁTICO (EletroNIC V1.0)
   ========================================================================== */

let lastCalculatedEquation = "";
console.log("EletroNIC Engine Carregada. Copyright © 2026 KMZ.");
...
function updateGateLogic() {
    ...
    led.style.boxShadow = res ? '0 0 20px var(--success)' : 'none';
    document.getElementById('logic-text').innerHTML = `${a?1:0} ${gate} ${gate==='NOT'?'':(b?1:0)} = ${res?1:0}`;
    
    let svg = `<svg viewBox="0 0 100 60" style="width:100%;height:100%"><rect x="30" y="15" width="40" height="30" rx="4" fill="none" stroke="var(--primary)" stroke-width="2"/><text x="50" y="34" fill="var(--primary)" font-weight="bold" font-size="12" text-anchor="middle">${gate}</text></svg>`;
...
function drawGateSimple(ctx, type, x, y) {
    ...
    if (type === 'AND' || type === 'NAND') {
        ...
        ctx.lineTo(x, y+15); ctx.lineTo(x, y+15); ctx.lineTo(x, y-15); 
        ctx.fill(); ctx.stroke();
...
// Inicializações padrão ao carregar
updateGateLogic(); 
generateTruthTable(); 
initKMapGrid();
